...
import { dummyCompanies, dummyAdmins } from "@/data/employeeData";
...
export default function SuperAdminPage() {
  ...
  const handleSaveCompany = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newComp: Company = {
      id: editingComp?.id || `COMP-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
      name: formData.get("name") as string,
      code: formData.get("code") as string,
      phone: formData.get("phone") as string,
      email: formData.get("email") as string,
      address: formData.get("address") as string,
      // จำนวนเหล่านี้จะไม่ถูกแก้ในฟอร์ม แต่จะดึงค่าเดิมมาแสดง (หรือ 0 สำหรับบริษัทใหม่)
      siteCount: editingComp?.siteCount || 0, 
      adminCount: editingComp?.adminCount || 0,
      leaderCount: editingComp?.leaderCount || 0,
      staffCount: editingComp?.staffCount || 0,
      status: editingComp?.status || "active",
      createdAt: editingComp?.createdAt || new Date().toISOString(),
    };
    if (editingComp) setCompanies(prev => prev.map(c => c.id === editingComp.id ? newComp : c));
    else setCompanies(prev => [newComp, ...prev]);
  ...
  const handleSaveAdmin = (e: React.FormEvent<HTMLFormElement>) => {
    ...
    const newAdmin: CompanyAdmin = {
      id: editingAdmin?.id || `ADM-${Math.floor(Math.random() * 1000).toString().padStart(3, '0')}`,
      companyId: formData.get("companyId") as string,
      name: formData.get("name") as string,
      username: formData.get("username") as string,
      email: formData.get("email") as string,
      password: formData.get("password") as string || "password123",
      avatar: editingAdmin?.avatar || `https://i.pravatar.cc/150?u=${Math.random()}`,
      status: editingAdmin?.status || "active",
      siteCount: editingAdmin?.siteCount || 0,
      leaderManaged: editingAdmin?.leaderManaged || 0,
      staffManaged: editingAdmin?.staffManaged || 0,
      createdAt: editingAdmin?.createdAt || new Date().toISOString(),
    };
    if (editingAdmin) setAdmins(prev => prev.map(a => a.id === editingAdmin.id ? newAdmin : a));
    else setAdmins(prev => [newAdmin, ...prev]);
    setShowAdminModal(false);
    setEditingAdmin(null);
  };

  const deleteItem = (id: string, type: 'comp' | 'admin') => {
    if (confirm("ยืนยันการลบข้อมูลนี้?")) {
      if (type === 'comp') setCompanies(prev => prev.filter(c => c.id !== id));
    ...
    <div className="min-h-screen bg-[#F1F5F9] text-slate-900 font-sans pb-10">
      ...
      <nav className="bg-white/80 backdrop-blur-md border-b border-slate-200 px-8 py-4 sticky top-0 z-50">
        <div className="max-w-[1600px] mx-auto flex justify-between items-center">
          ...
          <button onClick={() => router.push("/login")} className="flex items-center gap-2 bg-slate-100 hover:bg-red-50 hover:text-red-600 px-4 py-2 rounded-xl transition-all font-bold text-sm">
        ...
        <div className="fixed inset-0 bg-slate-900/60 backdrop-blur-sm z-[100] flex items-center justify-center p-4">
          <div className="bg-white rounded-[2rem] w-full max-w-lg overflow-hidden shadow-2xl animate-in fade-in zoom-in duration-200">
            ...
            <form onSubmit={handleSaveCompany} className="p-8 space-y-4">
              ...
              <button type="submit" className="w-full bg-indigo-600 text-white py-4 rounded-xl font-black shadow-lg shadow-indigo-100 hover:bg-indigo-700 transition-all mt-4">บันทึกข้อมูลบริษัท</button>
            </form>
          </div>
        </div>
      )}
            ...
            <form onSubmit={handleSaveAdmin} className="p-8 space-y-4">
              ...
              <div className="grid grid-cols-2 gap-4">
                ...
                <div className="space-y-1">
                  ...
                  <input name="password" type="password" placeholder="••••••••" className="w-full px-4 py-3 rounded-xl border border-slate-200 focus:ring-2 focus:ring-purple-500 outline-none font-bold" />
                </div>
              </div>
              <div className="space-y-1">
              ...
              <button type="submit" className="w-full bg-purple-600 text-white py-4 rounded-xl font-black shadow-lg shadow-purple-100 hover:bg-purple-700 transition-all mt-4">ยืนยันข้อมูลแอดมิน</button>
            </form>
          </div>
        </div>
      )}
