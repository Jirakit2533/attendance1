...
export default function SuperAdminClientPage({ 
  ...
  const handleDelete = async (id: string, name: string, type: 'comp' | 'admin') => {
    if (!window.confirm(`คุณแน่ใจหรือไม่ที่จะลบ ${type === 'comp' ? 'บริษัท' : 'แอดมิน'}: "${name}"?`)) return;
    setIsDeleting(id);
    try {
      const result = type === 'comp' ? await deleteCompanyAction(id) : await deleteAdminAction(id);
      if (result.success) {
        router.refresh();
      } else {
                ...
                <tr key={c.id} className="group hover:bg-indigo-50/30 transition-all">
                    ...
                    <div className="flex items-center gap-4">
                      ...
                      <div>
                        ...
                        <p className="text-[10px] text-slate-400 font-medium mt-1 uppercase tracking-wider">
                          ลงทะเบียนเมื่อ: {new Date(c.createdAt).toLocaleDateString('th-TH')}
                        </p>
                      </div>
                    </div>
                  </td>
                  ...
                  <td className="px-8 py-6 text-center">
                    <div className="flex justify-center gap-2 text-[10px]">                      
                      <span className="px-3 py-1 bg-indigo-50 text-indigo-600 rounded-lg w-fit">{c.adminCount} แอดมิน</span>
                      <span className="px-3 py-1 bg-purple-50 text-purple-600 rounded-lg w-fit">{c.leaderCount} หัวหน้า</span>
                      <span className="px-3 py-1 bg-slate-100 text-slate-600 rounded-lg w-fit">{c.employeeCount} พนักงาน</span>
                    </div>
                  </td>
                  <td className="px-8 py-6">
                  ...
                  <tr key={a.id} className="group hover:bg-purple-50/30 transition-all">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex flex-col">
                        ...
                        <span className="text-[10px] text-slate-400 uppercase tracking-tighter">Tenant ID: {a.companyId.slice(0,8)}...</span>
                      </div>
                    </td>
                    ...
                    <td className="px-8 py-6 text-center">
                      <div className="flex justify-center gap-2 text-[10px]">
                        <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded-md">{a.departmentCount} แผนก</span>
                        <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded-md">{a.siteCount} ไซต์</span>
                        <span className="bg-slate-100 text-slate-600 px-2 py-1 rounded-md">{a.positionCount} ตำแหน่ง</span>
                      </div>
                    </td>
                    <td className="px-8 py-6 text-center">
                      <div className="flex justify-center gap-2 text-[10px]">
                        <span className="bg-purple-100 text-purple-700 px-2 py-1 rounded-md">{a.leaderCount} หัวหน้า</span>
                        <span className="bg-emerald-100 text-emerald-700 px-2 py-1 rounded-md">{a.employeeCount} พนักงาน</span>
                      </div>
                    </td>
                    <td className="px-8 py-6">
                      <div className="flex justify-center items-center gap-2">
                        ...
                        <div className="text-left">
                          <p className="text-sm text-slate-700">{a.updateByName || a.createdByName || "System"}</p>
                          <p className="text-[10px] text-slate-400 font-normal italic">
                    ...
                    <td className="px-8 py-6">
                      <div className="flex justify-end gap-2 opacity-30 group-hover:opacity-100 transition-opacity">
                        <button 
                          onClick={() => { setEditingAdmin(a); setShowAdminModal(true); }} 
