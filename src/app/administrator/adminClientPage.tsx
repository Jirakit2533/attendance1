...
const filteredAttendance = useMemo(() => {
  ...
  const handleUpdateAdmin = async (formData: FormData) => {
    // 🛡️ 1. ป้องกันการกดซ้ำ (Double Click)
    if (isProcessing) return;

    try {
      setIsProcessing(true); // เริ่มต้นการทำงาน (Lock ปุ่ม)

      // 🛡️ 2. ดึงค่ามาเช็คเบื้องต้น (Client-side Validation)
      const currentPass = formData.get("currentPassword") as string;
      const newPass = formData.get("newPassword") as string;
      const confirmPass = formData.get("confirmPassword") as string;

      // บังคับต้องกรอกรหัสผ่านเดิมเสมอ
      ...
      // ถ้ามีการตั้งรหัสผ่านใหม่ ต้องเช็คให้ตรงกัน
      ...
      // 🚀 3. ส่งข้อมูลไปที่ Server Action (ตัวที่เราแก้เรื่อง transaction แล้ว)
      // มั่นใจว่าได้ import updateAdminProfileAction มาจาก actions.ts แล้ว
      const result = await updateAdminProfileAction(admin.id, formData);

      if (result.success) {
        alert("✅ " + (result.message || "บันทึกข้อมูลโปรไฟล์สำเร็จ!"));
        
        // ปิด Modal (เช็คชื่อตัวแปรของคุณว่าชื่อ setShowAdminEdit หรือไม่)
        if (typeof setShowAdminEdit === 'function') {
          setShowAdminEdit(false);
        }
        
        // ล้างรูป Preview (ถ้ามี)
        if (typeof setPreviewImage === 'function') {
          setPreviewImage(null);
        }
      } else {
        // แสดง Error จาก Server เช่น "รหัสผ่านเดิมไม่ถูกต้อง"
      ...
      setIsProcessing(false); // ปลด Lock ปุ่มให้กลับมาใช้งานได้
    }
  };
...
const handleAddSite = async (e: React.FormEvent<HTMLFormElement>) => {
  ...
  {/* ส่วนรูปภาพ: โชว์อย่างเดียว (หรือใส่ Effect เล็กน้อยตอน Hover) */}
  <div className="w-28 h-28 sm:w-32 sm:h-32 rounded-[2rem] bg-gradient-to-br from-blue-600 to-indigo-700 flex items-center justify-center text-white font-black shadow-xl shadow-blue-100 border-4 border-white overflow-hidden relative">
    {isUploading && (
  ...
  {/* จุดกดใหม่: เปลี่ยนที่คำว่า "โปรไฟล์แอดมิน" */}
  <button 
  type="button"
  onClick={() => setShowAdminEdit(true)} 
  className="mt-3 flex flex-col items-center group outline-none"
>
  <span className="text-[12px] font-black uppercase tracking-widest text-center text-slate-500 hover:text-blue-600 hover:bg-blue-50 transition-all duration-300 py-1 px-4 rounded-full">
    👤 แก้ไขโปรไฟล์แอดมิน
  </span>
  
  {/* เส้นใต้ Animation (แถมให้เผื่ออยากให้ดูรู้ว่าเป็นจุดกด) */}
  <div className="w-0 h-0.5 bg-blue-600 transition-all duration-300 group-hover:w-full opacity-50"></div>
</button>
</div>
    ...
    >
      ...
      <div className="grid grid-cols-1 md:grid-cols-2 gap-4">
        ...
        <div className="space-y-4">
          <h3 className="text-[11px] font-black text-blue-600 uppercase tracking-[0.2em] px-2">General Info</h3>
        ...
        <div className="space-y-4 bg-slate-50/50 p-4 rounded-[2rem] border border-slate-100">
          <div className="flex justify-between items-center px-2">
            <h3 className="text-[11px] font-black text-red-500 uppercase tracking-[0.2em]">Security Settings</h3>
            <button 
              type="button" 
              onClick={() => setShowPass(!showPass)}
              className="text-[10px] font-bold text-slate-400 hover:text-blue-600 transition-colors"
            >
              {showPass ? "🔒 ซ่อนรหัส" : "👁️ แสดงรหัส"}
            </button>
          </div>
          ...
          <div className="space-y-1 text-slate-900">
            ...
            <input 
              ...
              className="w-full bg-white p-4 rounded-2xl font-bold border border-slate-200 focus:border-red-500 outline-none transition-all" 
              placeholder="••••••••"
              required
            />
          </div>
          ...
          <div className="space-y-1 text-slate-900">
            ...
            <input 
              name="newPassword" 
              type={showPass ? "text" : "password"}
              className="w-full bg-white p-4 rounded-2xl font-bold border border-slate-200 focus:border-blue-500 outline-none transition-all" 
